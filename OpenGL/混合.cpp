...
#pragma endregion
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//面剔除：可以不渲染观察方向看不到的背面；提升大于50%的效率
		//====================================================================================================
		glEnable(GL_CULL_FACE);//启用面剔除，面剔除的立方体数据要确保都为正面（逆时针），否则部分面渲染会出错
		                       //且要在绘制完立方体后关闭，否则窗户无法从后面看到
		glCullFace(GL_BACK);//设置面剔除种类GL_BACK;GL_FORNT;GL_FRONT_AND_BACK分别为剔除背面;正面;两面
		glFrontFace(GL_CCW);//用于定义正面与背面；GL_CCW为逆时针正面GL_CW为顺时针正面；
        //====================================================================================================
		...
		glUniformMatrix4fv(glGetUniformLocation(shader.Program, "model"), 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
		glDisable(GL_CULL_FACE);//在此处关闭面剔除
		// Floor
		glBindVertexArray(planeVAO);
